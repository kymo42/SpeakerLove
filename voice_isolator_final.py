"""
Voice Isolator - FINAL VERSION
Real-time speaker output removal - Simplified and robust
"""
...
class GUI:
    ...
    def __init__(self, root):
        ...
        ttk.Label(title_frame, text="Voice Isolator", font=("Arial", 16, "bold")).pack()
        ttk.Label(title_frame, text="Remove speaker audio from microphone input", font=("Arial", 10)).pack()
        ...
        self.mic_combo = ttk.Combobox(dev_frame, textvariable=self.mic_var, state="readonly", width=60)
        ...
        ttk.Label(dev_frame, text="(Try: Stereo Mix, What U Hear, or Input (Voicemeeter...))", 
                 font=("Arial", 8), foreground="gray").grid(row=1, column=1, sticky=tk.W, padx=(0, 0))
        self.speaker_var = tk.StringVar()
        self.speaker_combo = ttk.Combobox(dev_frame, textvariable=self.speaker_var, state="readonly", width=60)
        self.speaker_combo.grid(row=2, column=1, sticky=tk.W)
        
        ttk.Label(dev_frame, text="Output Device:").grid(row=3, column=0, sticky=tk.W, pady=5)
        self.output_var = tk.StringVar()
        self.output_combo = ttk.Combobox(dev_frame, textvariable=self.output_var, state="readonly", width=60)
        ...
        self.status_label = ttk.Label(ctrl_frame, text="Status: STOPPED", foreground="red", font=("Arial", 12, "bold"))
    ...
    def refresh_devices(self):
        ...
        try:
            ...
            for i, dev in enumerate(devices):
                ...
                # Microphones - only real microphones
                if dev['max_input_channels'] > 0 and any(k in name_lower for k in ['microphone', 'mic', 'capture']):
                    if not any(k in name_lower for k in ['voicemeeter', 'what u hear']):
                        mics.append(dev_str)
                
                # Loopback/Input - ALL input devices (user will pick which one works)
            ...
            if speakers:
                # Try to find Stereo Mix as default
            ...
            print(f"Found {len(mics)} mics, {len(speakers)} input devices, {len(outputs)} outputs")
